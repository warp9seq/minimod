#!/usr/bin/env bash
#
# Usage: ./compare_freq_bed_bed.sh file1.bedmethyl file2.bedmethyl output_dir
#
...
f2=$2
out_dir=$3
# Check if extension of files are correct. file1 should be .bedmethyl and file2 should be .bedmethyl or .bed 
if [[ "$f1" != *.bedmethyl && "$f2" != *.bed ]] || [[ "$f2" != *.bedmethyl && "$f2" != *.bed ]]; then
    echo "Error: file1 and file2should have .bedmethyl or .bed extension."
    exit 1
fi
