#!/usr/bin/env bash
#
# Usage: ./compare_freq_bed_mmtsv.sh file1.bedmethyl file2.tsv output_dir
#
...
usage() {
    echo "Usage: $0 file1.bedmethyl file2.tsv output_dir"
    echo "Options:"
...
# --- Configuration ---
TOLERANCE=0.005 # Define the probability difference tolerance
# --- Argument Check ---
if [[ $# -ne 3 ]]; then
    usage
    exit 1
...
f2=$2
out_dir=$3
# # Check if extension of files are correct. file1 should be .bedmethyl or .bed and file2 should be .tsv
# if [[ "$f1" != *.bedmethyl && "$f1" != *.bed ]] || [[ "$f2" != *.tsv ]]; then
#     echo "Error: file1 should have .bedmethyl or .bed extension and file2 should have .tsv extension."
#     exit 1
# fi
...
OUT_LARGE_DIFF="$out_dir/large_freq_diff.tsv" # Keys in both with large freq difference
    ...
    # Second pass: process file2 and compare against the `prob` array
    {
        ...
        if (key1 in prob) {
            ...
            if (diff <= tol) {
                ...
                print key1 "\t" diff > out_large_diff
            }
