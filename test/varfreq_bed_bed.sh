...
# bed format (contig, start, end, mod, n_called, strand, start, end, color, m_called, freq, var_id, var_pos, var_gt, ref_base, alt_base, cg_offset)
# chr22	11211064	11211065	m	7	+	11211064	11211065	255,0,0	7	0.000000	.	11211064	0/1	T	C	0
# chr22	11211065	11211066	m	4	+	11211065	11211066	255,0,0	4	0.000000	.	11211064	0/1	T	C	0
# chr22	11211065	11211066	m	3	-	11211065	11211066	255,0,0	3	100.000000	.	11211064	0/1	T	C	0
...
# contig(1), start(2), end(3), mod(4), strand(6), ref_base(15), cg_offset(17)
key1=$(mktemp)
key2=$(mktemp)
trap 'rm -f "$key1" "$key2"' EXIT

awk 'BEGIN{OFS="\t"} {print $1,$2,$3,$4,$6,$15,$17}' "$f1" | sort -u > "$key1"
awk 'BEGIN{OFS="\t"} {print $1,$2,$3,$4,$6,$15,$17}' "$f2" | sort -u > "$key2"
