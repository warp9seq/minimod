#!/usr/bin/env bash
#
# Usage: ./compare_view_mmtsv_mmtsv.sh file1.tsv file2.tsv output_dir
#
...
# --- Configuration ---
TOLERANCE=0.001 # Define the probability difference tolerance
# --- Argument Check ---
if [[ $# -ne 3 ]]; then
    usage
    exit 1
...
if [[ "$f1" != *.tsv ]] || [[ "$f2" != *.tsv ]]; then
    echo "Error: Both files should have .tsv extension."
    exit 1
fi
